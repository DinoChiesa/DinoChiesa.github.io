...
// last saved: <2024-April-23 15:01:13>
...
(function () {
  ...
  // for localstorage
  const html5AppId =
    window.html5AppId || "43C9BB71-3E94-441C-B7F5-7FAE6FCD8458";
  const linkTemplate =
    window.linkTemplate ||
    "http://${edgeorg}-${edgeenv}.apigee.net/oauth2/v1/authorize?client_id=${clientid}&redirect_uri=${cburi}&response_type=${rtype}&state=${state}&scope=${scope}";

  function randomValue(len) {
    return Math.random()
      .toString(36)
      .substring(2, len ? len + 2 : 8);
  }
  ...
  function evalTemplate(template, model, encoder) {
    encoder = encoder || ((v) => v);
    let s = template;
    Object.keys(model)
      ...
      .forEach(function (key) {
        ...
        s = s.replace(pattern, encoder(value));
      });
    return s;
  }

  function updateLink() {
    const link = cleanDoubleSlash(
      evalTemplate(linkTemplate, model, encodeURIComponent)
    );
    ...
    if (model.code) {
      ...
      $("#preBox").text(
        "curl -X POST -H content-type:application/x-www-form-urlencoded -u " +
          ...
          model.clientsecret +
          " " +
          wrapInSingleQuote(newUrl) +
          " -d " +
          wrapInSingleQuote(payload)
      );
  ...
  function getFormItemValue($item) {
    ...
    switch (eltType) {
      case "INPUT":
        return $item.val();
      case "SELECT":
        return getFormSelectItemValue($item);
    }
    return null;
  }
  ...
  function resetRedemption(event) {
    ...
    $("#preBox").text("");
    $("#code").val("");
    $("#redeemResult").hide().removeClass("error").html("");
    updateModel();
    if (event) event.preventDefault();
  }
  ...
  function invokeRedemption(event) {
    ...
    $.ajax({
      ...
      headers: {
        Authorization:
          "Basic " + btoa(model.clientid + ":" + model.clientsecret)
      },
      data: payload,
